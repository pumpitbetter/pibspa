...
### 2025-09-08: Database Integration & Infrastructure

**Major Infrastructure Addition: PostgreSQL + Prisma Integration**
- Implemented dual database architecture (Server: PostgreSQL, Client: RxDB)
- Created comprehensive database setup with Docker containerization
- Added Prisma ORM with proper schema and seed data
- Implemented build isolation: SSR routes include database, SPA routes exclude database
- Added safe database access utilities with runtime environment checks
- Created automated database setup script (`scripts/setup-database.sh`)

**Database Features**
- **Schema**: Users, UserProfiles, Exercises, Workouts, WorkoutExercises, WorkoutSets
- **Seed Data**: 7 default exercises (Squat, Deadlift, Bench Press, Pull-ups, Push-ups, Running, Plank)
- **Development Tools**: Prisma Studio, pgAdmin, automated containerization
- **Safety**: Runtime checks prevent database access in browser/SPA environments

**Route Architecture**
- Database routes (`app/routes/exercises._index.tsx`) work in SSR builds only
- SPA builds automatically exclude database-dependent routes
- Utility functions (`app/lib/db/utils.ts`) provide safe database access patterns
- Environment-aware error handling for graceful degradation

**Infrastructure Files Added**
- `prisma/schema.prisma`: Database schema definition
- `prisma/seed.ts`: Default exercise data seeding
- `docker-compose.yml`: PostgreSQL + pgAdmin containerization
- `database/init/01-init.sql`: Database initialization
- `app/lib/db/prisma.ts`: Prisma client configuration
- `app/lib/db/utils.ts`: Safe database access utilities
